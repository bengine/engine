...
#define SKIP_TOKEN(parser) \
    ...
    parser->stream_end_produced = (parser->tokens.head->type == YAML_STREAM_END_TOKEN), parser->tokens.head ++)
...
static int yaml_parser_process_directives(yaml_parser_t *parser, yaml_version_t **version, yaml_tag_t **tag_start, yaml_tag_t **tag_end);
static int yaml_parser_append_tag_directive(yaml_parser_t *parser, yaml_tag_t value, int allow_duplicates, yaml_mark_t mark);
